...
let truthValue x fs = 
    let rec truthValueRec fs x = 
        ...
        | LeftNumber(a, b) -> 
            if x > b then 1.0
            else if x < a then 0.0
            else (x - a) / (b - a)
        | RightNumber(c, d) -> 
            if x < c then 1.0
            else if x > d then 0.0
            else 1.0 - (x - c) / (d - c)
        | TriangleNumber(a, b, d) -> 
            if x > b then truthValueRec (RightNumber(b, d)) x
            else truthValueRec (LeftNumber(a, b)) x
        | TrapezoidNumber(a, b, c, d) -> 
            if x > c then truthValueRec (RightNumber(c, d)) x
            else if x < b then truthValueRec (LeftNumber(a, b)) x
            else 1.0
    truthValueRec fs x
...
let inBound (x1, y1) (x2, y2) (x : float, y : float) = 
    ...
    (x > xl || Math.Abs(x - xl) < epsilon) && (x < xr || Math.Abs(x - xr) < epsilon) 
    && (y > yl || Math.Abs(y - yl) < epsilon) && (y < yh || Math.Abs(y - yh) < epsilon)
...
let intersection line1 line2 = 
    ...
    if denominator = 0.0 then None
        ...
        let p1, p2 = line1
        let p3, p4 = line2
        if inBound p1 p2 p && inBound p3 p4 p then Some p
        else None
...
let getMax fs = 
    ...
    | LeftNumber(a, b) -> b
    | RightNumber(c, d) -> c
    | TriangleNumber(a, b, d) -> b
    | TrapezoidNumber(a, b, c, d) -> b

let getLeftLine (fs, t) = 
    match fs with
    | LeftNumber(a, b) -> ((a, 0.0), (getX a b t, t))
    | TriangleNumber(a, b, d) -> ((a, 0.0), (getX a b t, t))
    | TrapezoidNumber(a, b, c, d) -> ((a, 0.0), (getX a b t, t))

let getTopLine (fs, t) = 
    match fs with
    | LeftNumber(a, b) -> ((getX a b t, t), (b, t))
    | RightNumber(c, d) -> ((c, t), (getX d c t, t))
    | TriangleNumber(a, b, d) -> ((getX a b t, t), (getX d b t, t))
    | TrapezoidNumber(a, b, c, d) -> ((getX a b t, t), (getX d c t, t))

let getRightLine (fs, t) = 
    match fs with
    | RightNumber(c, d) -> ((getX d c t, t), (d, 0.0))
    | TriangleNumber(a, b, d) -> ((getX d b t, t), (d, 0.0))
    | TrapezoidNumber(a, b, c, d) -> ((getX d c t, t), (d, 0.0))
    ...
    let rec unionRec li results = 
        ...
        | h1 :: tail -> 
            ...
            | h2 :: tail2 -> 
                ...
                match pi with
                | Some(xr, yr) -> 
                    List.append (points |> List.takeWhile (fun p -> fst p < xr)) [ (xr, yr) ] 
                    @ unionRec pi (h2 :: tail2)
                | None -> points @ unionRec pi (h2 :: tail2)
            | [] -> points
        | [] -> []
...
let centroid (x1 : float, y1 : float) (x2, y2) = 
    ...
    let ct = 
        if (y1 < y2) then (minX + 2.0 * dx / 3.0, h + dy / 3.0), dx * dy / 2.0
        else (minX + dx / 3.0, h + dy / 3.0), dx * dy / 2.0
...
let fireRules handleRule rules = 
    rules
    |> Seq.map handleRule
    |> Seq.map (fun (output, inputs) -> 
           output, 
           inputs
           |> Seq.map (fun (fs, x) -> truthValue x fs)
           |> Seq.min)
